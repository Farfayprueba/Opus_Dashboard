...
def clientes_inaccesibles():
    ...
    SELECT nombre
    FROM clientes
    WHERE Opus = 3 AND lugar = 'D1' AND estatus = 'Activo'
    """
...
def get_colaboradores_data():
    ...
    SELECT usuario
    FROM colaboradores
    WHERE statusData = 1 AND lugar = 'D1'
    """
    mydbs = mysql.connector.connect(
        host='rimgsa.com',
        port=3306,
...
def load_data():
    ...
    try:
        ...
        df = df[df['Pagina'].apply(lambda x: x.lower()).isin([cliente['cliente'].lower() for cliente in clientes if cliente['lugar'] == "D1"])]
...
layout = html.Div(
    children=[
        html.H1("Proyecto Opus Team D1"),
        html.H1(id="total_clientes-D1",
                style={'float': 'right',
                    "margin-right":"50px",
                    "font-size": "28px",
                    "font-weight": "bold",
        ...
        html.Div(style={'position': 'fixed', 'top': '0', 'left': '990px', 'width': '100%', 'height': '220px', 'overflowY': 'scroll', 'font-size': '40px', "font-weight": "bold",},
                children=[
                    dcc.Interval(
                    id='interval-component-d1',
                    interval=60*1000,  # Actualizar cada minuto
                    n_intervals=0
                ),
                dcc.Graph(id='colaboradores-data-d1', 
        ...
        html.Div(
                    ...
                    html.Div(
                            ...
                            children=[
                                ...
                                html.H1(id='countdown-d1', style={'font-size': '48px', 'margin-left': '30px', 'display': 'inlne-block',}),
                                html.P(id='message-d1', style={'font-size': '20px', 'margin-left': '30px'}),
                            ]
                        ),
        html.Div(id="clientes_inaccesibles_D1"),
        html.H2(""),
...
@app.callback(
    Output('total_clientes-D1', 'children'),
    Input('interval-component-d1', 'n_intervals')
)
...
@app.callback(
    Output('clientes_inaccesibles_D1', 'children'),
    Input('interval-component-d1', 'n_intervals')
)
def clientes(n):    
...
@app.callback(
    Output('colaboradores-data-d1', 'figure'),
    Input('interval-component-d1', 'n_intervals')
)
def update_colaboradores_data(n):
...
@app.callback(
    Output('clientes-data-D1', 'figure'),
    Input('interval-component-d1', 'n_intervals')
)
def update_colaboradores_data(n):
    df = cargar_clientes()
...
@app.callback(
    Output('button-agregar-d1', 'disabled'),  # Habilitar/deshabilitar el botón de agregar
    [Input('input-cliente-d1', 'value'),
     Input('input-likes-d1', 'value'),
     Input('input-comentarios-d1', 'value'),
     Input('input-compartidos-d1', 'value'),
     Input('input-ubicacion-d1','value')]
)
...
@app.callback(
    Output('clear-button-d1', 'disabled'),  # Disable the delete button if no row is selected
    [Input('data-table', 'selected_rows')]
)
def update_button_disabled2(selected_rows):
    return len(selected_rows) == 0

@app.callback(Output('data-table2-d1', 'data'),
              Input('interval-component', 'n_intervals'))
def update_data_table2(n_intervals):
    ...
    return updated_data
@app.callback(
    Output('input-cliente-d1', 'value'),  # Limpiar el campo de entrada
    Output('input-likes-d1', 'value'),  # Limpiar la selección de likes
    Output('input-comentarios-d1', 'value'),  # Limpiar la selección de comentarios
    Output('input-compartidos-d1', 'value'),
    Output('input-ubicacion-d1', 'value'),  # Limpiar la selección de compartidos
    [Input('button-agregar-d1', 'n_clicks'),
     Input('clear-button-d1', 'n_clicks')],  # Include clear-button-d1 as input trigger
...
@app.callback(
    Output('countdown-d1', 'children'),
    Output('message-d1', 'children'),
    Input('interval', 'n_intervals'),
    State('countdown-d1', 'children')
)
