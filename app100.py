        ...
        for row in reader:
            if len(row) >= 5:
                cliente = {
                    ...
                    "nombre": row[9],  #no se si hay algo aqui
                    "selectos": row[10]  ##tampoco se si hay algo aqui
                }
...
def load_data():
    ...
    try:
        ...
        especial = ["ESPECIAL"]
        df = df[df['Pagina'].apply(lambda x: x.lower()).isin([cliente['cliente'].lower() for cliente in clientes if cliente['selectos'] in especial])]
...
layout = html.Div(
    children=[
        ...
        html.Div(style={'position': 'fixed', 'top': '0', 'left': '990px', 'width': '100%', 'height': '220px', 'overflowY': 'scroll', 'font-size': '40px', "font-weight": "bold",},
                children=[
                    dcc.Interval(
                    id='interval-component-ejecutivo-K1',
                    interval=60*1000,  # Actualizar cada minuto
                    n_intervals=0
                ),
                dcc.Graph(id='colaboradores-data-ejecutivo-K1', 
        ...
        html.Div(
                    ...
                    html.Div(
                            ...
                            children=[
                                ...
                                html.H1(id='countdown-ejecutivo-K1', style={'font-size': '48px', 'margin-left': '30px', 'display': 'inlne-block',}),
                                html.P(id='message-ejecutivo-K1', style={'font-size': '20px', 'margin-left': '30px'}),
                            ]
                        ),
        html.H2(""),
...
@app.callback(
    Output('colaboradores-data-ejecutivo-K1', 'figure'),
    Input('interval-component-ejecutivo-K1', 'n_intervals')
)
...
@app.callback(
    Output('contador-ejecutivo-K1', 'children'),
...
@app.callback(
    Output('button-agregar-ejecutivo-K1', 'disabled'),  # Habilitar/deshabilitar el botón de agregar
    [Input('input-cliente-ejecutivo-K1', 'value'),
     Input('input-likes-ejecutivo-K1', 'value'),
     Input('input-comentarios-ejecutivo-K1', 'value'),
     Input('input-compartidos-ejecutivo-K1', 'value'),
     Input('input-ubicacion-ejecutivo-K1','value')]
)
...
@app.callback(
    Output('clear-button-ejecutivo-K1', 'disabled'),  # Disable the delete button if no row is selected
    [Input('data-table', 'selected_rows')]
)
def update_button_disabled2(selected_rows):
    return len(selected_rows) == 0

 @app.callback(Output('data-table2-ejecutivo-K1', 'data'),
              Input('interval-component', 'n_intervals'))
def update_data_table2(n_intervals):
    ...
    return updated_data
@app.callback(
    Output('input-cliente-ejecutivo-K1', 'value'),  # Limpiar el campo de entrada
    Output('input-likes-ejecutivo-K1', 'value'),  # Limpiar la selección de likes
    Output('input-comentarios-ejecutivo-K1', 'value'),  # Limpiar la selección de comentarios
    Output('input-compartidos-ejecutivo-K1', 'value'),
    Output('input-ubicacion-ejecutivo-K1', 'value'),  # Limpiar la selección de compartidos
    [Input('button-agregar-ejecutivo-K1', 'n_clicks'),
     Input('clear-button-ejecutivo-K1', 'n_clicks')],  # Include clear-button-ejecutivo-K as input trigger
...
@app.callback(
    Output('countdown-ejecutivo-K1', 'children'),
    Output('message-ejecutivo-K1', 'children'),
    Input('interval', 'n_intervals'),
    State('countdown-ejecutivo-K1', 'children')
) 
