...
def load_data():
    ...
    try:
        ...
        df = df[df['Pagina'].apply(lambda x: x.lower()).isin([cliente['cliente'].lower() for cliente in clientes if cliente['lugar'] == "HERCULES"])]
...
layout = html.Div(
    children=[
        html.H1("Proyecto Opus Team Hercules"),
        ...
        html.Div(
                    ...
                    html.Div(
                            ...
                            children=[
                                ...
                                html.H1(id='countdown-hercules', style={'font-size': '48px', 'margin-left': '30px', 'display': 'inlne-block',}),
                                html.P(id='message-hercules', style={'font-size': '20px', 'margin-left': '30px'}),
                            ]
                        ),
        html.H2(""),
...
@app.callback(
    Output('contador-hercules', 'children'),
...
@app.callback(
    Output('button-agregar-hercules', 'disabled'),  # Habilitar/deshabilitar el botón de agregar
    [Input('input-cliente-hercules', 'value'),
     Input('input-likes-hercules', 'value'),
     Input('input-comentarios-hercules', 'value'),
     Input('input-compartidos-hercules', 'value'),
     Input('input-ubicacion-hercules','value')]
)
...
@app.callback(
    Output('clear-button-hercules', 'disabled'),  # Disable the delete button if no row is selected
    [Input('data-table', 'selected_rows')]
)
def update_button_disabled2(selected_rows):
    return len(selected_rows) == 0

@app.callback(Output('data-table2-hercules', 'data'),
              Input('interval-component', 'n_intervals'))
def update_data_table2(n_intervals):
    ...
    return updated_data
@app.callback(
    Output('input-cliente-hercules', 'value'),  # Limpiar el campo de entrada
    Output('input-likes-hercules', 'value'),  # Limpiar la selección de likes
    Output('input-comentarios-hercules', 'value'),  # Limpiar la selección de comentarios
    Output('input-compartidos-hercules', 'value'),
    Output('input-ubicacion-hercules', 'value'),  # Limpiar la selección de compartidos
    [Input('button-agregar-hercules', 'n_clicks'),
     Input('clear-button-hercules', 'n_clicks')],  # Include clear-button-hercules as input trigger
...
@app.callback(
    Output('countdown-hercules', 'children'),
    Output('message-hercules', 'children'),
    Input('interval', 'n_intervals'),
    State('countdown-hercules', 'children')
)
