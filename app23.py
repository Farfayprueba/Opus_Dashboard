...
def get_colaboradores_data():
    ...
    SELECT usuario
    FROM colaboradores
    WHERE statusData = 1 AND lugar LIKE '%D%'
    """
    mydbs = mysql.connector.connect(
        host='rimgsa.com',
        port=3306,
...
def load_data():
    ...
    try:
        ...
        lugares_filtrar = ["D1", "D2", "D3"]

        # Filtrar el DataFrame
        df = df[df['Pagina'].apply(lambda x: x.lower()).isin([cliente['cliente'].lower() for cliente in clientes if cliente['lugar'] in lugares_filtrar])]
...
layout = html.Div(
    children=[
        html.H1("Proyecto Opus Team D"),
        html.H1(id="total_clientes-D",
                style={'float': 'right',
                    "margin-right":"50px",
                    "font-size": "28px",
                    "font-weight": "bold",
        ...
        html.Div(style={'position': 'fixed', 'top': '0', 'left': '990px', 'width': '100%', 'height': '220px', 'overflowY': 'scroll', 'font-size': '40px', "font-weight": "bold",},
                children=[
                    dcc.Interval(
                    id='interval-component-D',
                    interval=60*1000,  # Actualizar cada minuto
                    n_intervals=0
                ),
                dcc.Graph(id='colaboradores-data-D', 
        ...
        html.Div(
                    ...
                    html.Div(
                            ...
                            children=[
                                ...
                                html.H1(id='countdown-d', style={'font-size': '48px', 'margin-left': '30px', 'display': 'inlne-block',}),
                                html.P(id='message-d', style={'font-size': '20px', 'margin-left': '30px'}),
                            ]
                        ),
        html.Div(id="clientes_inaccesibles_D"),
        html.H2(""),
...
@app.callback(
    Output('total_clientes-D', 'children'),
    Input('interval-component-D', 'n_intervals')
)
def update_colaboradores_data(n):
    df = cargar_clientes()
    df = pd.DataFrame(df)
    df = df[df["lugar"].str.contains("D")]
    
    return html.Div(f"Total de clientes: {len(df)} \n")

@app.callback(
    Output('clientes_inaccesibles_D', 'children'),
    Input('interval-component-D', 'n_intervals')
)
def clientes(n):    
...
@app.callback(
    Output('colaboradores-data-D', 'figure'),
    Input('interval-component-D', 'n_intervals')
)
def update_colaboradores_data(n):
...
@app.callback(
    Output('button-agregar-d', 'disabled'),  # Habilitar/deshabilitar el botón de agregar
    [Input('input-cliente-d', 'value'),
     Input('input-likes-d', 'value'),
     Input('input-comentarios-d', 'value'),
     Input('input-compartidos-d', 'value'),
     Input('input-ubicacion-d','value')]
)
...
@app.callback(
    Output('clear-button-d', 'disabled'),  # Disable the delete button if no row is selected
    [Input('data-table', 'selected_rows')]
)
def update_button_disabled2(selected_rows):
    return len(selected_rows) == 0

@app.callback(Output('data-table2-d', 'data'),
              Input('interval-component', 'n_intervals'))
...
@app.callback(
    Output('input-cliente-d', 'value'),  # Limpiar el campo de entrada
    Output('input-likes-d', 'value'),  # Limpiar la selección de likes
    Output('input-comentarios-d', 'value'),  # Limpiar la selección de comentarios
    Output('input-compartidos-d', 'value'),
    Output('input-ubicacion-d', 'value'),  # Limpiar la selección de compartidos
    [Input('button-agregar-d', 'n_clicks'),
     Input('clear-button-d', 'n_clicks')],  # Include clear-button-d as input trigger
...
@app.callback(
    Output('countdown-d', 'children'),
    Output('message-d', 'children'),
    Input('interval', 'n_intervals'),
    State('countdown-d', 'children')
)
