...
def clientes_inaccesibles():
    ...
    SELECT nombre
    FROM clientes
    WHERE Opus = 3 AND enlace = 'Enlace_Aliz'
    """
...
def get_colaboradores_data():
    ...
    SELECT usuario
    FROM colaboradores
    WHERE statusData = 1 AND lugar = 'ejecutivo-A'
    """
    mydbs = mysql.connector.connect(
        host='rimgsa.com',
        port=3306,
...
def load_data():
    ...
    try:
        ...
        df = df[df['Pagina'].apply(lambda x: x.lower()).isin([cliente['cliente'].lower() for cliente in clientes if cliente['enlace'] == "Enlace_Aliz"])]
...
layout = html.Div(
    children=[
        html.H1("Proyecto Opus Ejecutivo Alizbeth"),
        ...
        html.Div(style={'position': 'fixed', 'top': '0', 'left': '990px', 'width': '100%', 'height': '220px', 'overflowY': 'scroll', 'font-size': '40px', "font-weight": "bold",},
                children=[
                    dcc.Interval(
                    id='interval-component-ejecutivo-A',
                    interval=60*1000,  # Actualizar cada minuto
                    n_intervals=0
                ),
                dcc.Graph(id='colaboradores-data-ejecutivo-A', 
        ...
        html.Div(
                    ...
                    html.Div(
                            ...
                            children=[
                                ...
                                html.H1(id='countdown-ejecutivo-A', style={'font-size': '48px', 'margin-left': '30px', 'display': 'inlne-block',}),
                                html.P(id='message-ejecutivo-A', style={'font-size': '20px', 'margin-left': '30px'}),
                            ]
                        ),
        
        html.Div(id="clientes_inaccesibles_alice"),
        html.H2(""),
...
@app.callback(
    Output('clientes_inaccesibles_alice', 'children'),
    Input('interval-component-ejecutivo-A', 'n_intervals')
)
def clientes(n):    
...
@app.callback(
    Output('colaboradores-data-ejecutivo-A', 'figure'),
    Input('interval-component-ejecutivo-A', 'n_intervals')
)
...
@app.callback(
    Output('contador-ejecutivo-A', 'children'),
...
@app.callback(
    Output('button-agregar-ejecutivo-A', 'disabled'),  # Habilitar/deshabilitar el botón de agregar
    [Input('input-cliente-ejecutivo-A', 'value'),
     Input('input-likes-ejecutivo-A', 'value'),
     Input('input-comentarios-ejecutivo-A', 'value'),
     Input('input-compartidos-ejecutivo-A', 'value'),
     Input('input-ubicacion-ejecutivo-A','value')]
)
...
@app.callback(
    Output('clear-button-ejecutivo-A', 'disabled'),  # Disable the delete button if no row is selected
    [Input('data-table', 'selected_rows')]
)
def update_button_disabled2(selected_rows):
    return len(selected_rows) == 0

@app.callback(Output('data-table2-ejecutivo-A', 'data'),
              Input('interval-component', 'n_intervals'))
def update_data_table2(n_intervals):
    ...
    return updated_data
@app.callback(
    Output('input-cliente-ejecutivo-A', 'value'),  # Limpiar el campo de entrada
    Output('input-likes-ejecutivo-A', 'value'),  # Limpiar la selección de likes
    Output('input-comentarios-ejecutivo-A', 'value'),  # Limpiar la selección de comentarios
    Output('input-compartidos-ejecutivo-A', 'value'),
    Output('input-ubicacion-ejecutivo-A', 'value'),  # Limpiar la selección de compartidos
    [Input('button-agregar-ejecutivo-A', 'n_clicks'),
     Input('clear-button-ejecutivo-A', 'n_clicks')],  # Include clear-button-ejecutivo-A as input trigger
...
@app.callback(
    Output('countdown-ejecutivo-A', 'children'),
    Output('message-ejecutivo-A', 'children'),
    Input('interval', 'n_intervals'),
    State('countdown-ejecutivo-A', 'children')
)
