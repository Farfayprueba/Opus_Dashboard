...
def load_data():
    ...
    try:
        ...
        #df = df[df['Pagina'].apply(lambda x: x.lower()).isin([cliente['cliente'].lower() for cliente in clientes if cliente['lugar'] == "HERCULES1" | cliente['lugar'] == "HERCULES2" | cliente['lugar'] == "HERCULES3" | cliente['lugar'] == "HERCULES4" ])]
        df = df[df['Pagina'].apply(lambda x: x.lower()).isin([cliente['cliente'].lower() for cliente in clientes if cliente['lugar'] in ["HERCULES1", "HERCULES2", "HERCULES3", "HERCULES4"]])]
...
layout = html.Div(
    children=[
        html.H1("Proyecto Opus Team Hercules Global"),
        ...
        html.Div(
                    ...
                    html.Div(
                            ...
                            children=[
                                ...
                                html.H1(id='countdown-hercules-global', style={'font-size': '48px', 'margin-left': '30px', 'display': 'inlne-block',}),
                                html.P(id='message-hercules-global', style={'font-size': '20px', 'margin-left': '30px'}),
                            ]
                        ),
        html.H2(""),
            ...
            children=[
                ...
                html.Div(
                    dash_table.DataTable(
                        id="data-table2-hercules-global",
                        columns=columns,
...
@app.callback(
    Output('contador-hercules-global', 'children'),
...
@app.callback(
    Output('button-agregar-hercules-global', 'disabled'),  # Habilitar/deshabilitar el botón de agregar
    [Input('input-cliente-hercules-global', 'value'),
     Input('input-likes-hercules-global', 'value'),
     Input('input-comentarios-hercules-global', 'value'),
     Input('input-compartidos-hercules-global', 'value'),
     Input('input-ubicacion-hercules-global','value')]
)
...
@app.callback(
    Output('clear-button-hercules-global', 'disabled'),  # Disable the delete button if no row is selected
    [Input('data-table', 'selected_rows')]
)
def update_button_disabled2(selected_rows):
    return len(selected_rows) == 0

@app.callback(Output('data-table2-hercules-global', 'data'),
              Input('interval-component', 'n_intervals'))
def update_data_table2(n_intervals):
    ...
    return updated_data
@app.callback(
    Output('input-cliente-hercules-global', 'value'),  # Limpiar el campo de entrada
    Output('input-likes-hercules-global', 'value'),  # Limpiar la selección de likes
    Output('input-comentarios-hercules-global', 'value'),  # Limpiar la selección de comentarios
    Output('input-compartidos-hercules-global', 'value'),
    Output('input-ubicacion-hercules-global', 'value'),  # Limpiar la selección de compartidos
    [Input('button-agregar-hercules-global', 'n_clicks'),
     Input('clear-button-hercules-global', 'n_clicks')],  # Include clear-button-hercules-global as input trigger
...
@app.callback(
    Output('countdown-hercules-global', 'children'),
    Output('message-hercules-global', 'children'),
    Input('interval', 'n_intervals'),
    State('countdown-hercules-global', 'children')
)
