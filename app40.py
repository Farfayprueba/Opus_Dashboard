...
def clientes_inaccesibles():
    ...
    SELECT nombre
    FROM clientes
    WHERE Opus = 3 AND lugar = 'D6' AND estatus = 'Activo'
    """
...
def get_colaboradores_data():
    ...
    SELECT usuario
    FROM colaboradores
    WHERE statusData = 1 AND lugar = 'D6'
    """
    mydbs = mysql.connector.connect(
        host='rimgsa.com',
        port=3306,
...
def load_data():
    ...
    try:
        ...
        df = df[df['Pagina'].apply(lambda x: x.lower()).isin([cliente['cliente'].lower() for cliente in clientes if cliente['lugar'] == "D6"])]
...
layout = html.Div(
    children=[
        html.H1("Proyecto Opus Team D6"),
        html.H1(id="total_clientes-D6", #cambiar id
                style={'float': 'right',
                    "margin-right":"50px",
                    "font-size": "28px",
                    "font-weight": "bold",
        ...
        html.Div(style={'position': 'fixed', 'top': '0', 'left': '990px', 'width': '100%', 'height': '220px', 'overflowY': 'scroll', 'font-size': '40px', "font-weight": "bold",},
                children=[
                    dcc.Interval(
                    id='interval-component-D6',
                    interval=60*1000,  # Actualizar cada minuto
                    n_intervals=0
                ),
                dcc.Graph(id='colaboradores-data-D6', 
        ...
        html.Div(
                    ...
                    html.Div(
                            ...
                            children=[
                                ...
                                html.H1(id='countdown-D6', style={'font-size': '48px', 'margin-left': '30px', 'display': 'inlne-block',}),
                                html.P(id='message-D6', style={'font-size': '20px', 'margin-left': '30px'}),
                            ]
                        ),
        html.Div(id="clientes_inaccesibles_D6"),
        html.H2(""),
...
#____________________________________________________________________________________________________
@app.callback(
    Output('total_clientes-D6', 'children'),
    Input('interval-component-D6', 'n_intervals')
)
...
@app.callback(
    Output('clientes_inaccesibles_D6', 'children'),
    Input('interval-component-D6', 'n_intervals')
)
def clientes(n):    
...
#____________________________________________________________________________________________________
@app.callback(
    Output('colaboradores-data-D6', 'figure'),
    Input('interval-component-D6', 'n_intervals')
)
def update_colaboradores_data(n):
...
@app.callback(
    Output('clientes-data-D6', 'figure'),
    Input('interval-component-D6', 'n_intervals')
)
def update_colaboradores_data(n):
    df = cargar_clientes()
...
@app.callback(
    Output('button-agregar-D6', 'disabled'),  # Habilitar/deshabilitar el botón de agregar
    [Input('input-cliente-D6', 'value'),
     Input('input-likes-D6', 'value'),
     Input('input-comentarios-D6', 'value'),
     Input('input-compartidos-D6', 'value'),
     Input('input-ubicacion-D6','value')]
)
...
@app.callback(
    Output('clear-button-D6', 'disabled'),  # Disable the delete button if no row is selected
    [Input('data-table', 'selected_rows')]
)
def update_button_disabled2(selected_rows):
    return len(selected_rows) == 0

@app.callback(Output('data-table2-D6', 'data'),
              Input('interval-component', 'n_intervals'))
def update_data_table2(n_intervals):
    ...
    return updated_data
@app.callback(
    Output('input-cliente-D6', 'value'),  # Limpiar el campo de entrada
    Output('input-likes-D6', 'value'),  # Limpiar la selección de likes
    Output('input-comentarios-D6', 'value'),  # Limpiar la selección de comentarios
    Output('input-compartidos-D6', 'value'),
    Output('input-ubicacion-D6', 'value'),  # Limpiar la selección de compartidos
    [Input('button-agregar-D6', 'n_clicks'),
     Input('clear-button-D6', 'n_clicks')],  # Include clear-button-D6 as input trigger
...
@app.callback(
    Output('countdown-D6', 'children'),
    Output('message-D6', 'children'),
    Input('interval', 'n_intervals'),
    State('countdown-D6', 'children')
)
