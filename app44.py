...
def clientes_inaccesibles():
    ...
    SELECT nombre
    FROM clientes
    WHERE Opus = 3 AND enlace = 'Enlace_Amilcar'
    """
...
def get_colaboradores_data():
    ...
    SELECT usuario
    FROM colaboradores
    WHERE statusData = 1 AND lugar = 'ejecutivo-Amilcar'
    """
...
def load_data():
    ...
    try:
        ...
        df = df[df['Pagina'].apply(lambda x: x.lower()).isin([cliente['cliente'].lower() for cliente in clientes if cliente['enlace'] == "Enlace_Amilcar"])]
...
layout = html.Div(
    children=[
        html.H1("Proyecto Opus Ejecutivo Amilcar"),
        ...
        html.Div(style={'position': 'fixed', 'top': '0', 'left': '990px', 'width': '100%', 'height': '220px', 'overflowY': 'scroll', 'font-size': '40px', "font-weight": "bold",},
                children=[
                    dcc.Interval(
                    id='interval-component-ejecutivo-Amilcar',
                    interval=60*1000,  # Actualizar cada minuto
                    n_intervals=0
                ),
                dcc.Graph(id='colaboradores-data-ejecutivo-Amilcar', 
        ...
        html.Div(
                    ...
                    html.Div(
                            ...
                            children=[
                                ...
                                html.H1(id='countdown-ejecutivo-Amilcar', style={'font-size': '48px', 'margin-left': '30px', 'display': 'inlne-block',}),
                                html.P(id='message-ejecutivo-Amilcar', style={'font-size': '20px', 'margin-left': '30px'}),
                            ]
                        ),
        html.Div(id="clientes_inaccesibles_Amilcar"),
            ...
            children=[
                ...
                html.Div(
                    dash_table.DataTable(
                        id="data-table2-ejecutivo-Amilcar",
                        columns=columns,
...
@app.callback(
    Output('colaboradores-data-ejecutivo-Amilcar', 'figure'),
    Input('interval-component-ejecutivo-Amilcar', 'n_intervals')
)
...
@app.callback(
    Output('clientes_inaccesibles_Amilcar', 'children'),
    Input('interval-component-ejecutivo-Amilcar', 'n_intervals')
)
def clientes(n):    
...
@app.callback(
    Output('contador-ejecutivo-Amilcar', 'children'),
...
@app.callback(
    Output('button-agregar-ejecutivo-Amilcar', 'disabled'),  # Habilitar/deshabilitar el botón de agregar
    [Input('input-cliente-ejecutivo-Amilcar', 'value'),
     Input('input-likes-ejecutivo-Amilcar', 'value'),
     Input('input-comentarios-ejecutivo-Amilcar', 'value'),
     Input('input-compartidos-ejecutivo-Amilcar', 'value'),
     Input('input-ubicacion-ejecutivo-Amilcar','value')]
)
...
@app.callback(
    Output('clear-button-ejecutivo-Amilcar', 'disabled'),  # Disable the delete button if no row is selected
    [Input('data-table', 'selected_rows')]
)
def update_button_disabled2(selected_rows):
    return len(selected_rows) == 0

@app.callback(Output('data-table2-ejecutivo-Amilcar', 'data'),
              Input('interval-component', 'n_intervals'))
def update_data_table2(n_intervals):
    ...
    return updated_data
@app.callback(
    Output('input-cliente-ejecutivo-Amilcar', 'value'),  # Limpiar el campo de entrada
    Output('input-likes-ejecutivo-Amilcar', 'value'),  # Limpiar la selección de likes
    Output('input-comentarios-ejecutivo-Amilcar', 'value'),  # Limpiar la selección de comentarios
    Output('input-compartidos-ejecutivo-Amilcar', 'value'),
    Output('input-ubicacion-ejecutivo-Amilcar', 'value'),  # Limpiar la selección de compartidos
    [Input('button-agregar-ejecutivo-Amilcar', 'n_clicks'),
     Input('clear-button-ejecutivo-Amilcar', 'n_clicks')],  # Include clear-button-ejecutivo-Amilcar as input trigger
...
@app.callback(
    Output('countdown-ejecutivo-Amilcar', 'children'),
    Output('message-ejecutivo-Amilcar', 'children'),
    Input('interval', 'n_intervals'),
    State('countdown-ejecutivo-Amilcar', 'children')
)
