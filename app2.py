...
def load_data():
    ...
    try:
        ...
        df = df[df['Pagina'].apply(lambda x: x.lower()).isin([cliente['cliente'].lower() for cliente in clientes if cliente['lugar'] == "MASARIK"])]
...
layout = html.Div(
    children=[
        html.H1("Proyecto Opus Team Masarik"),
        ...
        html.Div(
                    ...
                    html.Div(
                            ...
                            children=[
                                ...
                                html.H1(id='countdown-masarik', style={'font-size': '48px', 'margin-left': '30px', 'display': 'inlne-block',}),
                                html.P(id='message-masarik', style={'font-size': '20px', 'margin-left': '30px'}),
                            ]
                        ),
        html.H2(""),
...
@app.callback(
    Output('contador-masarik', 'children'),
...
@app.callback(
    Output('button-agregar-masarik', 'disabled'),  # Habilitar/deshabilitar el botón de agregar
    [Input('input-cliente-masarik', 'value'),
     Input('input-likes-masarik', 'value'),
     Input('input-comentarios-masarik', 'value'),
     Input('input-compartidos-masarik', 'value'),
     Input('input-ubicacion-masarik','value')]
)
...
@app.callback(
    Output('clear-button-masarik', 'disabled'),  # Disable the delete button if no row is selected
    [Input('data-table', 'selected_rows')]
)
def update_button_disabled2(selected_rows):
    return len(selected_rows) == 0

@app.callback(Output('data-table2-masarik', 'data'),
              Input('interval-component', 'n_intervals'))
def update_data_table2(n_intervals):
    ...
    return updated_data
@app.callback(
    Output('input-cliente-masarik', 'value'),  # Limpiar el campo de entrada
    Output('input-likes-masarik', 'value'),  # Limpiar la selección de likes
    Output('input-comentarios-masarik', 'value'),  # Limpiar la selección de comentarios
    Output('input-compartidos-masarik', 'value'),
    Output('input-ubicacion-masarik', 'value'),  # Limpiar la selección de compartidos
    [Input('button-agregar-masarik', 'n_clicks'),
     Input('clear-button-masarik', 'n_clicks')],  # Include clear-button-masarik as input trigger
...
@app.callback(
    Output('countdown-masarik', 'children'),
    Output('message-masarik', 'children'),
    Input('interval', 'n_intervals'),
    State('countdown-masarik', 'children')
)
