...
def clientes_inaccesibles():
    ...
    SELECT nombre
    FROM clientes
    WHERE Opus = 3 AND lugar = 'HERCULES1' AND estatus = 'Activo'
    """
...
def get_colaboradores_data():
    ...
    SELECT usuario
    FROM colaboradores
    WHERE statusData = 1 AND lugar = 'hercules1'
    """
    mydbs = mysql.connector.connect(
        host='rimgsa.com',
        port=3306,
...
def load_data():
    ...
    try:
        ...
        df = df[df['Pagina'].apply(lambda x: x.lower()).isin([cliente['cliente'].lower() for cliente in clientes if cliente['lugar'] == "HERCULES1"])]
...
layout = html.Div(
    children=[
        html.H1("Proyecto Opus Team Hercules (1)"),
        #____________________________________________________________________________________________________
        html.H1(id="total_clientes-hercules1", #cambiar id
                style={'float': 'right',
                    "margin-right":"50px",
                    "font-size": "28px",
                    "font-weight": "bold",
        ...
        html.Div(style={'position': 'fixed', 'top': '0', 'left': '990px', 'width': '100%', 'height': '220px', 'overflowY': 'scroll', 'font-size': '40px', "font-weight": "bold",},
                children=[
                    dcc.Interval(
                    id='interval-component-hercules1',
                    interval=60*1000,  # Actualizar cada minuto
                    n_intervals=0
                ),
                dcc.Graph(id='colaboradores-data-hercules1', 
        ...
        html.Div(style={'position': 'fixed', 'top': '0', 'left': '800px', 'width': '100%', 'height': '220px', 'overflowY': 'scroll', 'font-size': '40px', "font-weight": "bold",},
                children=[
                dcc.Graph(id='clientes-data-hercules1', 
                          config={'displayModeBar': False},
                          style={'width': '10%', "font-weight": "bold", 'font-size': '40px'} ),
            ]),
        #_________________________________________________________________________________________________
        html.Div(
                    ...
                    html.Div(
                            ...
                            children=[
                                ...
                                html.H1(id='countdown-hercules-(1)', style={'font-size': '48px', 'margin-left': '30px', 'display': 'inlne-block',}),
                                html.P(id='message-hercules-(1)', style={'font-size': '20px', 'margin-left': '30px'}),
                            ]
                        ),
        html.Div(id="clientes_inaccesibles_hercules1"),
        html.H2(""),
...
#____________________________________________________________________________________________________
@app.callback(
    Output('total_clientes-hercules1', 'children'),
    Input('interval-component-hercules1', 'n_intervals')
)
def update_colaboradores_data(n):
    df = cargar_clientes()
    df = pd.DataFrame(df)
    df = df[df["lugar"] == "HERCULES1"]
    
    return html.Div(f"Total de clientes: {len(df)} \n")
#____________________________________________________________________________________________________

@app.callback(
    Output('clientes_inaccesibles_hercules1', 'children'),
    Input('interval-component-hercules1', 'n_intervals')
)
def clientes(n):    
...
#____________________________________________________________________________________________________
@app.callback(
    Output('colaboradores-data-hercules1', 'figure'),
    Input('interval-component-hercules1', 'n_intervals')
)
def update_colaboradores_data(n):
...
#____________________________________________________________________________________________________
@app.callback(
    Output('clientes-data-hercules1', 'figure'),
    Input('interval-component-hercules1', 'n_intervals')
)
def update_colaboradores_data(n):
    df = cargar_clientes()
...
#____________________________________________________________________________________________________
@app.callback(
    Output('contador-hercules-(1)', 'children'),
...
@app.callback(
    Output('button-agregar-hercules-(1)', 'disabled'),  # Habilitar/deshabilitar el botón de agregar
    [Input('input-cliente-hercules-(1)', 'value'),
     Input('input-likes-hercules-(1)', 'value'),
     Input('input-comentarios-hercules-(1)', 'value'),
     Input('input-compartidos-hercules-(1)', 'value'),
     Input('input-ubicacion-hercules-(1)','value')]
)
...
@app.callback(
    Output('clear-button-hercules-(1)', 'disabled'),  # Disable the delete button if no row is selected
    [Input('data-table', 'selected_rows')]
)
def update_button_disabled2(selected_rows):
    return len(selected_rows) == 0

@app.callback(Output('data-table2-hercules-(1)', 'data'),
              Input('interval-component', 'n_intervals'))
def update_data_table2(n_intervals):
    ...
    return updated_data
@app.callback(
    Output('input-cliente-hercules-(1)', 'value'),  # Limpiar el campo de entrada
    Output('input-likes-hercules-(1)', 'value'),  # Limpiar la selección de likes
    Output('input-comentarios-hercules-(1)', 'value'),  # Limpiar la selección de comentarios
    Output('input-compartidos-hercules-(1)', 'value'),
    Output('input-ubicacion-hercules-(1)', 'value'),  # Limpiar la selección de compartidos
    [Input('button-agregar-hercules-(1)', 'n_clicks'),
     Input('clear-button-hercules-(1)', 'n_clicks')],  # Include clear-button-hercules-(1) as input trigger
...
@app.callback(
    Output('countdown-hercules-(1)', 'children'),
    Output('message-hercules-(1)', 'children'),
    Input('interval', 'n_intervals'),
    State('countdown-hercules-(1)', 'children')
)
