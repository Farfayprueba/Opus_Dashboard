...
def clientes_inaccesibles():
    ...
    SELECT nombre
    FROM clientes
    WHERE Opus = 3 AND lugar = 'C3' AND estatus = 'Activo'
    """
...
def get_colaboradores_data():
    ...
    SELECT usuario
    FROM colaboradores
    WHERE statusData = 1 AND lugar = 'C3'
    """
    mydbs = mysql.connector.connect(
        host='rimgsa.com',
        port=3306,
...
def load_data():
    ...
    try:
        ...
        df = df[df['Pagina'].apply(lambda x: x.lower()).isin([cliente['cliente'].lower() for cliente in clientes if cliente['lugar'] == "C3"])]
...
layout = html.Div(
    children=[
        html.H1("Proyecto Opus Team C3"),
        html.H1(id="total_clientes-C3",
                style={'float': 'right',
                    "margin-right":"50px",
                    "font-size": "28px",
                    "font-weight": "bold",
        ...
        html.Div(style={'position': 'fixed', 'top': '0', 'left': '990px', 'width': '100%', 'height': '220px', 'overflowY': 'scroll', 'font-size': '40px', "font-weight": "bold",},
                children=[
                    dcc.Interval(
                    id='interval-component-C3',
                    interval=60*1000,  # Actualizar cada minuto
                    n_intervals=0
                ),
                dcc.Graph(id='colaboradores-data-C3', 
        ...
        html.Div(
                    ...
                    html.Div(
                            ...
                            children=[
                                ...
                                html.H1(id='countdown-C3', style={'font-size': '48px', 'margin-left': '30px', 'display': 'inlne-block',}),
                                html.P(id='message-C3', style={'font-size': '20px', 'margin-left': '30px'}),
                            ]
                        ),
        html.Div(id="clientes_inaccesibles_C3"),
        html.H2(""),
...
@app.callback(
    Output('total_clientes-C3', 'children'),
    Input('interval-component-C3', 'n_intervals')
)
...
@app.callback(
    Output('clientes_inaccesibles_C3', 'children'),
    Input('interval-component-C3', 'n_intervals')
)
def clientes(n):    
...
@app.callback(
    Output('colaboradores-data-C3', 'figure'),
    Input('interval-component-C3', 'n_intervals')
)
def update_colaboradores_data(n):
...
@app.callback(
    Output('clientes-data-C3', 'figure'),
    Input('interval-component-C3', 'n_intervals')
)
def update_colaboradores_data(n):
    df = cargar_clientes()
...
@app.callback(
    Output('button-agregar-C3', 'disabled'),  # Habilitar/deshabilitar el botón de agregar
    [Input('input-cliente-C3', 'value'),
     Input('input-likes-C3', 'value'),
     Input('input-comentarios-C3', 'value'),
     Input('input-compartidos-C3', 'value'),
     Input('input-ubicacion-C3','value')]
)
...
@app.callback(
    Output('clear-button-C3', 'disabled'),  # Disable the delete button if no row is selected
    [Input('data-table', 'selected_rows')]
)
def update_button_disabled2(selected_rows):
    return len(selected_rows) == 0

@app.callback(Output('data-table2-C3', 'data'),
              Input('interval-component', 'n_intervals'))
...
@app.callback(
    Output('input-cliente-C3', 'value'),  # Limpiar el campo de entrada
    Output('input-likes-C3', 'value'),  # Limpiar la selección de likes
    Output('input-comentarios-C3', 'value'),  # Limpiar la selección de comentarios
    Output('input-compartidos-C3', 'value'),
    Output('input-ubicacion-C3', 'value'),  # Limpiar la selección de compartidos
    [Input('button-agregar-C3', 'n_clicks'),
     Input('clear-button-C3', 'n_clicks')],  # Include clear-button-C3 as input trigger
...
@app.callback(
    Output('countdown-C3', 'children'),
    Output('message-C3', 'children'),
    Input('interval', 'n_intervals'),
    State('countdown-C3', 'children')
)
