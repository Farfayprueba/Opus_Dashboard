...
def load_data():
    ...
    try:
        ...
        df = df[df['Pagina'].apply(lambda x: x.lower()).isin([cliente['cliente'].lower() for cliente in clientes if cliente['lugar'] == "D"])]
...
@app.callback(
    Output('contador-mini-d-equipo', 'children'),
...
@app.callback(
    Output('button-agregar-mini-d-equipo', 'disabled'),  # Habilitar/deshabilitar el botón de agregar
    [Input('input-cliente-mini-d-equipo', 'value'),
     Input('input-likes-mini-d-equipo', 'value'),
     Input('input-comentarios-mini-d-equipo', 'value'),
     Input('input-compartidos-mini-d-equipo', 'value'),
     Input('input-ubicacion-mini-d-equipo','value')]
)
...
@app.callback(
    Output('clear-button-mini-d-equipo', 'disabled'),  # Disable the delete button if no row is selected
    [Input('data-table', 'selected_rows')]
)
def update_button_disabled2(selected_rows):
    return len(selected_rows) == 0

@app.callback(Output('data-table2-mini-d-equipo', 'data'),
              Input('interval-component', 'n_intervals'))
def update_data_table2(n_intervals):
    ...
    return updated_data
@app.callback(
    Output('input-cliente-mini-d-equipo', 'value'),  # Limpiar el campo de entrada
    Output('input-likes-mini-d-equipo', 'value'),  # Limpiar la selección de likes
    Output('input-comentarios-mini-d-equipo', 'value'),  # Limpiar la selección de comentarios
    Output('input-compartidos-mini-d-equipo', 'value'),
    Output('input-ubicacion-mini-d-equipo', 'value'),  # Limpiar la selección de compartidos
    [Input('button-agregar-mini-d-equipo', 'n_clicks'),
     Input('clear-button-mini-d-equipo', 'n_clicks')],  # Include clear-button-mini-d-equipo as input trigger
...
@app.callback(
    Output('countdown-mini-d-equipo', 'children'),
    Output('message-mini-d-equipo', 'children'),
    Input('interval', 'n_intervals'),
    State('countdown-mini-d-equipo', 'children')
)
