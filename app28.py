...
def clientes_inaccesibles():
    ...
    SELECT nombre
    FROM clientes
    WHERE Opus = 3 AND enlace = 'Enlace_Raquel'
    """
...
def get_colaboradores_data():
    ...
    SELECT usuario
    FROM colaboradores
    WHERE statusData = 1 AND lugar = 'ejecutivo-R'
    """
    mydbs = mysql.connector.connect(
        host='rimgsa.com',
        port=3306,
...
def load_data():
    ...
    try:
        ...
        df = df[df['Pagina'].apply(lambda x: x.lower()).isin([cliente['cliente'].lower() for cliente in clientes if cliente['enlace'] == "Enlace_Raquel"])]
...
layout = html.Div(
    children=[
        html.H1("Proyecto Opus Ejecutivo Raquel"),
        ...
        html.Div(style={'position': 'fixed', 'top': '0', 'left': '990px', 'width': '100%', 'height': '220px', 'overflowY': 'scroll', 'font-size': '40px', "font-weight": "bold",},
                children=[
                    dcc.Interval(
                    id='interval-component-ejecutivo-R',
                    interval=60*1000,  # Actualizar cada minuto
                    n_intervals=0
                ),
                dcc.Graph(id='colaboradores-data-ejecutivo-R', 
        ...
        html.Div(
                    ...
                    html.Div(
                            ...
                            children=[
                                ...
                                html.H1(id='countdown-ejecutivo-R', style={'font-size': '48px', 'margin-left': '30px', 'display': 'inlne-block',}),
                                html.P(id='message-ejecutivo-R', style={'font-size': '20px', 'margin-left': '30px'}),
                            ]
                        ),
        html.Div(id="clientes_inaccesibles_raquel"),
        html.H2(""),
...
@app.callback(
    Output('clientes_inaccesibles_raquel', 'children'),
    Input('interval-component-ejecutivo-R', 'n_intervals')
)
def clientes(n):    
...
@app.callback(
    Output('colaboradores-data-ejecutivo-R', 'figure'),
    Input('interval-component-ejecutivo-R', 'n_intervals')
)
...
@app.callback(
    Output('contador-ejecutivo-R', 'children'),
...
@app.callback(
    Output('button-agregar-ejecutivo-R', 'disabled'),  # Habilitar/deshabilitar el botón de agregar
    [Input('input-cliente-ejecutivo-R', 'value'),
     Input('input-likes-ejecutivo-R', 'value'),
     Input('input-comentarios-ejecutivo-R', 'value'),
     Input('input-compartidos-ejecutivo-R', 'value'),
     Input('input-ubicacion-ejecutivo-R','value')]
)
...
@app.callback(
    Output('clear-button-ejecutivo-R', 'disabled'),  # Disable the delete button if no row is selected
    [Input('data-table', 'selected_rows')]
)
def update_button_disabled2(selected_rows):
    return len(selected_rows) == 0

@app.callback(Output('data-table2-ejecutivo-R', 'data'),
              Input('interval-component', 'n_intervals'))
def update_data_table2(n_intervals):
    ...
    return updated_data
@app.callback(
    Output('input-cliente-ejecutivo-R', 'value'),  # Limpiar el campo de entrada
    Output('input-likes-ejecutivo-R', 'value'),  # Limpiar la selección de likes
    Output('input-comentarios-ejecutivo-R', 'value'),  # Limpiar la selección de comentarios
    Output('input-compartidos-ejecutivo-R', 'value'),
    Output('input-ubicacion-ejecutivo-R', 'value'),  # Limpiar la selección de compartidos
    [Input('button-agregar-ejecutivo-R', 'n_clicks'),
     Input('clear-button-ejecutivo-R', 'n_clicks')],  # Include clear-button-ejecutivo-R as input trigger
...
@app.callback(
    Output('countdown-ejecutivo-R', 'children'),
    Output('message-ejecutivo-R', 'children'),
    Input('interval', 'n_intervals'),
    State('countdown-ejecutivo-R', 'children')
)
