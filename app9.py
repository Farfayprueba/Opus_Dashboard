...
def load_data():
    ...
    try:
        ...
        df = df[df['Pagina'].apply(lambda x: x.lower()).isin([cliente['cliente'].lower() for cliente in clientes if cliente['lugar'] == "HONDURAS"])]
...
layout = html.Div(
    children=[
        html.H1("Proyecto Opus Team Honduras"),
        ...
        html.Div(
                    ...
                    html.Div(
                            ...
                            children=[
                                ...
                                html.H1(id='countdown-honduras', style={'font-size': '48px', 'margin-left': '30px', 'display': 'inlne-block',}),
                                html.P(id='message-honduras', style={'font-size': '20px', 'margin-left': '30px'}),
                            ]
                        ),
        html.H2(""),
...
@app.callback(
    Output('contador-honduras', 'children'),
...
@app.callback(
    Output('button-agregar-honduras', 'disabled'),  # Habilitar/deshabilitar el botón de agregar
    [Input('input-cliente-honduras', 'value'),
     Input('input-likes-honduras', 'value'),
     Input('input-comentarios-honduras', 'value'),
     Input('input-compartidos-honduras', 'value'),
     Input('input-ubicacion-honduras','value')]
)
...
@app.callback(
    Output('clear-button-honduras', 'disabled'),  # Disable the delete button if no row is selected
    [Input('data-table', 'selected_rows')]
)
def update_button_disabled2(selected_rows):
    return len(selected_rows) == 0

@app.callback(Output('data-table2-honduras', 'data'),
              Input('interval-component', 'n_intervals'))
def update_data_table2(n_intervals):
    ...
    return updated_data
@app.callback(
    Output('input-cliente-honduras', 'value'),  # Limpiar el campo de entrada
    Output('input-likes-honduras', 'value'),  # Limpiar la selección de likes
    Output('input-comentarios-honduras', 'value'),  # Limpiar la selección de comentarios
    Output('input-compartidos-honduras', 'value'),
    Output('input-ubicacion-honduras', 'value'),  # Limpiar la selección de compartidos
    [Input('button-agregar-honduras', 'n_clicks'),
     Input('clear-button-honduras', 'n_clicks')],  # Include clear-button-honduras as input trigger
...
@app.callback(
    Output('countdown-honduras', 'children'),
    Output('message-honduras', 'children'),
    Input('interval', 'n_intervals'),
    State('countdown-honduras', 'children')
)
