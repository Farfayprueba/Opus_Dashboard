...
def clientes_inaccesibles():
    ...
    SELECT nombre
    FROM clientes
    WHERE Opus = 3 AND enlace = 'Enlace_Adriana'
    """
...
def get_colaboradores_data():
    ...
    SELECT usuario
    FROM colaboradores
    WHERE statusData = 1 AND lugar = 'ejecutivo-Adriana'
    """
    mydbs = mysql.connector.connect(
        host='rimgsa.com',
        port=3306,
...
def load_data():
    ...
    try:
        ...
        df = df[df['Pagina'].apply(lambda x: x.lower()).isin([cliente['cliente'].lower() for cliente in clientes if cliente['enlace'] == "Enlace_Adriana"])]
...
layout = html.Div(
    children=[
        html.H1("Proyecto Opus Ejecutivo Adriana"),
        ...
        html.Div(style={'position': 'fixed', 'top': '0', 'left': '990px', 'width': '100%', 'height': '220px', 'overflowY': 'scroll', 'font-size': '40px', "font-weight": "bold",},
                children=[
                    dcc.Interval(
                    id='interval-component-ejecutivo-Adriana',
                    interval=60*1000,  # Actualizar cada minuto
                    n_intervals=0
                ),
                dcc.Graph(id='colaboradores-data-ejecutivo-Adriana', 
        ...
        html.Div(
                    ...
                    html.Div(
                            ...
                            children=[
                                ...
                                html.H1(id='countdown-ejecutivo-Adriana', style={'font-size': '48px', 'margin-left': '30px', 'display': 'inlne-block',}),
                                html.P(id='message-ejecutivo-Adriana', style={'font-size': '20px', 'margin-left': '30px'}),
                            ]
                        ),
        html.Div(id="clientes_inaccesibles_adriana"),
        html.H2(""),
            ...
            children=[
                ...
                html.Div(
                    dash_table.DataTable(
                        id="data-table2-ejecutivo-Adriana",
                        columns=columns,
...
@app.callback(
    Output('clientes_inaccesibles_adriana', 'children'),
    Input('interval-component-ejecutivo-Adriana', 'n_intervals')
)
def clientes(n):    
...
@app.callback(
    Output('colaboradores-data-ejecutivo-Adriana', 'figure'),
    Input('interval-component-ejecutivo-Adriana', 'n_intervals')
)
...
@app.callback(
    Output('contador-ejecutivo-Adriana', 'children'),
...
@app.callback(
    Output('button-agregar-ejecutivo-Adriana', 'disabled'),  # Habilitar/deshabilitar el botón de agregar
    [Input('input-cliente-ejecutivo-Adriana', 'value'),
     Input('input-likes-ejecutivo-Adriana', 'value'),
     Input('input-comentarios-ejecutivo-Adriana', 'value'),
     Input('input-compartidos-ejecutivo-Adriana', 'value'),
     Input('input-ubicacion-ejecutivo-Adriana','value')]
)
...
@app.callback(
    Output('clear-button-ejecutivo-Adriana', 'disabled'),  # Disable the delete button if no row is selected
    [Input('data-table', 'selected_rows')]
)
def update_button_disabled2(selected_rows):
    return len(selected_rows) == 0

@app.callback(Output('data-table2-ejecutivo-Adriana', 'data'),
              Input('interval-component', 'n_intervals'))
def update_data_table2(n_intervals):
    ...
    return updated_data
@app.callback(
    Output('input-cliente-ejecutivo-Adriana', 'value'),  # Limpiar el campo de entrada
    Output('input-likes-ejecutivo-Adriana', 'value'),  # Limpiar la selección de likes
    Output('input-comentarios-ejecutivo-Adriana', 'value'),  # Limpiar la selección de comentarios
    Output('input-compartidos-ejecutivo-Adriana', 'value'),
    Output('input-ubicacion-ejecutivo-Adriana', 'value'),  # Limpiar la selección de compartidos
    [Input('button-agregar-ejecutivo-Adriana', 'n_clicks'),
     Input('clear-button-ejecutivo-Adriana', 'n_clicks')],  # Include clear-button-ejecutivo-Adriana as input trigger
...
@app.callback(
    Output('countdown-ejecutivo-Adriana', 'children'),
    Output('message-ejecutivo-Adriana', 'children'),
    Input('interval', 'n_intervals'),
    State('countdown-ejecutivo-Adriana', 'children')
)
