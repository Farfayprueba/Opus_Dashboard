...
def clientes_inaccesibles():
    ...
    SELECT nombre
    FROM clientes
    WHERE Opus = 3 AND enlace = 'Enlace_Erick'
    """
...
def load_data():
    ...
    try:
        ...
        df = df[df['Pagina'].apply(lambda x: x.lower()).isin([cliente['cliente'].lower() for cliente in clientes if cliente['enlace'] == "Enlace_Erick"])]
...
layout = html.Div(
    children=[
        html.H1("Proyecto Opus Ejecutivo Erick"),
        ...
        html.Div(style={'position': 'fixed', 'top': '0', 'left': '990px', 'width': '100%', 'height': '220px', 'overflowY': 'scroll', 'font-size': '40px', "font-weight": "bold",},
                children=[
                    dcc.Interval(
                    id='interval-component-ejecutivo-Erick',
                    interval=60*1000,  # Actualizar cada minuto
                    n_intervals=0
                ),
                dcc.Graph(id='colaboradores-data-ejecutivo-Erick', 
        ...
        html.Div(
                    ...
                    html.Div(
                            ...
                            children=[
                                ...
                                html.H1(id='countdown-ejecutivo-Erick', style={'font-size': '48px', 'margin-left': '30px', 'display': 'inlne-block',}),
                                html.P(id='message-ejecutivo-Erick', style={'font-size': '20px', 'margin-left': '30px'}),
                            ]
                        ),
        html.Div(id="clientes_inaccesibles_erick"),
            ...
            children=[
                ...
                html.Div(
                    dash_table.DataTable(
                        id="data-table2-ejecutivo-Erick",
                        columns=columns,
...
@app.callback(
    Output('colaboradores-data-ejecutivo-Erick', 'figure'),
    Input('interval-component-ejecutivo-Erick', 'n_intervals')
)
...
@app.callback(
    Output('clientes_inaccesibles_erick', 'children'),
    Input('interval-component-ejecutivo-Erick', 'n_intervals')
)
def clientes(n):    
...
@app.callback(
    Output('contador-ejecutivo-Erick', 'children'),
...
@app.callback(
    Output('button-agregar-ejecutivo-Erick', 'disabled'),  # Habilitar/deshabilitar el botón de agregar
    [Input('input-cliente-ejecutivo-Erick', 'value'),
     Input('input-likes-ejecutivo-Erick', 'value'),
     Input('input-comentarios-ejecutivo-Erick', 'value'),
     Input('input-compartidos-ejecutivo-Erick', 'value'),
     Input('input-ubicacion-ejecutivo-Erick','value')]
)
...
@app.callback(
    Output('clear-button-ejecutivo-Erick', 'disabled'),  # Disable the delete button if no row is selected
    [Input('data-table', 'selected_rows')]
)
def update_button_disabled2(selected_rows):
    return len(selected_rows) == 0

@app.callback(Output('data-table2-ejecutivo-Erick', 'data'),
              Input('interval-component', 'n_intervals'))
def update_data_table2(n_intervals):
    ...
    return updated_data
@app.callback(
    Output('input-cliente-ejecutivo-Erick', 'value'),  # Limpiar el campo de entrada
    Output('input-likes-ejecutivo-Erick', 'value'),  # Limpiar la selección de likes
    Output('input-comentarios-ejecutivo-Erick', 'value'),  # Limpiar la selección de comentarios
    Output('input-compartidos-ejecutivo-Erick', 'value'),
    Output('input-ubicacion-ejecutivo-Erick', 'value'),  # Limpiar la selección de compartidos
    [Input('button-agregar-ejecutivo-Erick', 'n_clicks'),
     Input('clear-button-ejecutivo-Erick', 'n_clicks')],  # Include clear-button-ejecutivo-Erick as input trigger
...
@app.callback(
    Output('countdown-ejecutivo-Erick', 'children'),
    Output('message-ejecutivo-Erick', 'children'),
    Input('interval', 'n_intervals'),
    State('countdown-ejecutivo-Erick', 'children')
)
