...
def clientes_inaccesibles():
    ...
    SELECT nombre
    FROM clientes
    WHERE Opus = 3 AND lugar = 'CANCUN' AND estatus = 'Activo'
    """
...
def get_colaboradores_data():
    ...
    SELECT usuario
    FROM colaboradores
    WHERE statusData = 1 AND lugar = 'cancun'
    """
    mydbs = mysql.connector.connect(
        host='rimgsa.com',
        port=3306,
...
def load_data():
    ...
    try:
        ...
        df = df[df['Pagina'].apply(lambda x: x.lower()).isin([cliente['cliente'].lower() for cliente in clientes if cliente['lugar'] == "CANCUN"])]
...
layout = html.Div(
    children=[
        html.H1("Proyecto Opus Team Cancun A"),
        html.H1(id="total_clientes-CANCUN",
                style={'float': 'right',
                    "margin-right":"50px",
                    "font-size": "28px",
                    "font-weight": "bold",
        ...
        html.Div(style={'position': 'fixed', 'top': '0', 'left': '990px', 'width': '100%', 'height': '220px', 'overflowY': 'scroll', 'font-size': '40px', "font-weight": "bold",},
                children=[
                    dcc.Interval(
                    id='interval-component-cancun',
                    interval=60*1000,  # Actualizar cada minuto
                    n_intervals=0
                ),
                dcc.Graph(id='colaboradores-data-cancun', 
        ...
        html.Div(style={'position': 'fixed', 'top': '0', 'left': '800px', 'width': '100%', 'height': '220px', 'overflowY': 'scroll', 'font-size': '40px', "font-weight": "bold",},
                children=[
                dcc.Graph(id='clientes-data-cancun', 
                          config={'displayModeBar': False},
                          style={'width': '10%', "font-weight": "bold", 'font-size': '40px'} ),
        ...
        html.Div(
                    ...
                    html.Div(
                            ...
                            children=[
                                ...
                                html.H1(id='countdown-cancun', style={'font-size': '48px', 'margin-left': '30px', 'display': 'inlne-block',}),
                                html.P(id='message-cancun', style={'font-size': '20px', 'margin-left': '30px'}),
                            ]
                        ),
        html.Div(id="clientes_inaccesibles_cancun"),
        html.H2(""),
...
@app.callback(
    Output('total_clientes-CANCUN', 'children'),
    Input('interval-component-cancun', 'n_intervals')
)
def update_colaboradores_data(n):
    df = cargar_clientes()
    df = pd.DataFrame(df)
    df = df[df["lugar"] == "CANCUN"]
    
    return html.Div(f"Total de clientes: {len(df)} \n")

@app.callback(
    Output('clientes_inaccesibles_cancun', 'children'),
    Input('interval-component-cancun', 'n_intervals')
)
def clientes(n):    
...
@app.callback(
    Output('colaboradores-data-cancun', 'figure'),
    Input('interval-component-cancun', 'n_intervals')
)
def update_colaboradores_data(n):
...
@app.callback(
    Output('clientes-data-cancun', 'figure'),
    Input('interval-component-cancun', 'n_intervals')
)
def update_colaboradores_data(n):
    df = cargar_clientes()
...
@app.callback(
    Output('contador-cancun', 'children'),
...
@app.callback(
    Output('button-agregar-cancun', 'disabled'),  
    [Input('input-cliente-cancun', 'value'),
     Input('input-likes-cancun', 'value'),
     Input('input-comentarios-cancun', 'value'),
     Input('input-compartidos-cancun', 'value'),
     Input('input-ubicacion-cancun','value')]
)
...
@app.callback(
    Output('clear-button-cancun', 'disabled'), 
    [Input('data-table', 'selected_rows')]
)
def update_button_disabled2(selected_rows):
    return len(selected_rows) == 0

@app.callback(Output('data-table2-cancun', 'data'),
              Input('interval-component', 'n_intervals'))
...
@app.callback(
    Output('input-cliente-cancun', 'value'),  
    Output('input-likes-cancun', 'value'),  
    Output('input-comentarios-cancun', 'value'),
    Output('input-compartidos-cancun', 'value'),
    Output('input-ubicacion-cancun', 'value'), 
    [Input('button-agregar-cancun', 'n_clicks'),
     Input('clear-button-cancun', 'n_clicks')], 
...
@app.callback(
    Output('countdown-cancun', 'children'),
    Output('message-cancun', 'children'),
    Input('interval', 'n_intervals'),
    State('countdown-cancun', 'children')
)
