...
def clientes_inaccesibles():
    ...
    SELECT nombre
    FROM clientes
    WHERE Opus = 3 AND lugar = 'CANCUN2' 
    """
...
def get_colaboradores_data():
    ...
    SELECT usuario
    FROM colaboradores
    WHERE statusData = 1 AND lugar = 'cancun2'
    """
...
def delete_rows(df):
    ...
    for cliente in clientes_guardados:
        ...
        #                                  (row["Comentarios"] >= comentarios_goal), axis=1)]
        rows_to_delete = df[df.apply(lambda row: (row["Pagina"].lower() == cliente["nombre"].lower()) and
...
def load_data():
    ...
    try:
        ...
        df = df[df['Pagina'].apply(lambda x: x.lower()).isin([cliente['cliente'].lower() for cliente in clientes if cliente['lugar'] == "CANCUN2"])]
...
layout = html.Div(
    children=[
        html.H1("Proyecto Opus Team Cancun B"),
        html.H1(id="total_clientes-CANCUN2",
                style={'float': 'right',
                    "margin-right":"50px",
                    "font-size": "28px",
                    "font-weight": "bold",
        ...
        html.Div(style={'position': 'fixed', 'top': '0', 'left': '990px', 'width': '100%', 'height': '220px', 'overflowY': 'scroll', 'font-size': '40px', "font-weight": "bold",},
                children=[
                    dcc.Interval(
                    id='interval-component-cancun2',
                    interval=60*1000,
                    n_intervals=0
                ),
                dcc.Graph(id='colaboradores-data-cancun2', 
        ...
        html.Div(style={'position': 'fixed', 'top': '0', 'left': '800px', 'width': '100%', 'height': '220px', 'overflowY': 'scroll', 'font-size': '40px', "font-weight": "bold",},
                children=[
                dcc.Graph(id='clientes-data-cancun2', 
                          config={'displayModeBar': False},
                          style={'width': '10%', "font-weight": "bold", 'font-size': '40px'} ),
            ]),
        html.Div(
                    ...
                    html.Div(
                            ...
                            children=[
                                ...
                                html.H1(id='countdown-cancun2', style={'font-size': '48px', 'margin-left': '30px', 'display': 'inlne-block',}),
                                html.P(id='message-cancun2', style={'font-size': '20px', 'margin-left': '30px'}),
                            ]
                        ),
        
        html.Div(id="clientes_inaccesibles_cancun2"),
        html.H2(""),
...
@app.callback(
    Output('total_clientes-CANCUN2', 'children'),
    Input('interval-component-cancun2', 'n_intervals')
)
def update_colaboradores_data(n):
    df = cargar_clientes()
    df = pd.DataFrame(df)
    df = df[df["lugar"] == "CANCUN2"]
    
    return html.Div(f"Total de clientes: {len(df)} \n")


@app.callback(
    Output('clientes_inaccesibles_cancun2', 'children'),
    Input('interval-component-cancun2', 'n_intervals')
)
def clientes(n):    
...
@app.callback(
    Output('colaboradores-data-cancun2', 'figure'),
    Input('interval-component-cancun2', 'n_intervals')
)
def update_colaboradores_data(n):
...
@app.callback(
    Output('clientes-data-cancun2', 'figure'),
    Input('interval-component-cancun2', 'n_intervals')
)
def update_colaboradores_data(n):
    df = cargar_clientes()
...
@app.callback(
    Output('contador-cancun2', 'children'),
...
@app.callback(
    Output('button-agregar-cancun2', 'disabled'),  # Habilitar/deshabilitar el botón de agregar
    [Input('input-cliente-cancun2', 'value'),
     Input('input-likes-cancun2', 'value'),
     Input('input-comentarios-cancun2', 'value'),
     Input('input-compartidos-cancun2', 'value'),
     Input('input-ubicacion-cancun2','value')]
)
...
@app.callback(
    Output('clear-button-cancun2', 'disabled'),  # Disable the delete button if no row is selected
    [Input('data-table', 'selected_rows')]
)
def update_button_disabled2(selected_rows):
    return len(selected_rows) == 0

@app.callback(Output('data-table2-cancun2', 'data'),
              Input('interval-component', 'n_intervals'))
def update_data_table2(n_intervals):
    ...
    return updated_data
@app.callback(
    Output('input-cliente-cancun2', 'value'),  # Limpiar el campo de entrada
    Output('input-likes-cancun2', 'value'),  # Limpiar la selección de likes
    Output('input-comentarios-cancun2', 'value'),  # Limpiar la selección de comentarios
    Output('input-compartidos-cancun2', 'value'),
    Output('input-ubicacion-cancun2', 'value'),  # Limpiar la selección de compartidos
    [Input('button-agregar-cancun2', 'n_clicks'),
     Input('clear-button-cancun2', 'n_clicks')],  # Include clear-button-cancun2 as input trigger
...
@app.callback(
    Output('countdown-cancun2', 'children'),
    Output('message-cancun2', 'children'),
    Input('interval', 'n_intervals'),
    State('countdown-cancun2', 'children')
)
