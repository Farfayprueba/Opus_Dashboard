            ...
            children=[
                html.Div(
                    dash_table.DataTable(
                        id="data-table2--mini-hercules-global",
                        columns=columns,
...
@app.callback(
    Output('contador--mini-hercules-global', 'children'),
...
@app.callback(
    Output('button-agregar--mini-hercules-global', 'disabled'),  # Habilitar/deshabilitar el botón de agregar
    [Input('input-cliente--mini-hercules-global', 'value'),
     Input('input-likes--mini-hercules-global', 'value'),
     Input('input-comentarios--mini-hercules-global', 'value'),
     Input('input-compartidos--mini-hercules-global', 'value'),
     Input('input-ubicacion--mini-hercules-global','value')]
)
...
@app.callback(
    Output('clear-button--mini-hercules-global', 'disabled'),  # Disable the delete button if no row is selected
    [Input('data-table', 'selected_rows')]
)
def update_button_disabled2(selected_rows):
    return len(selected_rows) == 0

@app.callback(Output('data-table2--mini-hercules-global', 'data'),
              Input('interval-component', 'n_intervals'))
def update_data_table2(n_intervals):
    ...
    return updated_data
@app.callback(
    Output('input-cliente--mini-hercules-global', 'value'),  # Limpiar el campo de entrada
    Output('input-likes--mini-hercules-global', 'value'),  # Limpiar la selección de likes
    Output('input-comentarios--mini-hercules-global', 'value'),  # Limpiar la selección de comentarios
    Output('input-compartidos--mini-hercules-global', 'value'),
    Output('input-ubicacion--mini-hercules-global', 'value'),  # Limpiar la selección de compartidos
    [Input('button-agregar--mini-hercules-global', 'n_clicks'),
     Input('clear-button--mini-hercules-global', 'n_clicks')],  # Include clear-button--mini-hercules-global as input trigger
...
@app.callback(
    Output('countdown--mini-hercules-global', 'children'),
    Output('message--mini-hercules-global', 'children'),
    Input('interval', 'n_intervals'),
    State('countdown--mini-hercules-global', 'children')
)
