...
def clientes_inaccesibles():
    ...
    SELECT nombre
    FROM clientes
    WHERE Opus = 3 AND lugar = 'HERCULES2' AND estatus = 'Activo'
    """
...
def get_colaboradores_data():
    ...
    SELECT usuario
    FROM colaboradores
    WHERE statusData = 1 AND lugar = 'hercules2'
    """
    mydbs = mysql.connector.connect(
        host='rimgsa.com',
        port=3306,
...
def load_data():
    ...
    try:
        ...
        df = df[df['Pagina'].apply(lambda x: x.lower()).isin([cliente['cliente'].lower() for cliente in clientes if cliente['lugar'] == "HERCULES2"])]
...
layout = html.Div(
    children=[
        html.H1("Proyecto Opus Team Hercules (2)"),
        html.H1(id="total_clientes-hercules2",
                style={'float': 'right',
                    "margin-right":"50px",
                    "font-size": "28px",
                    "font-weight": "bold",
        ...
        html.Div(style={'position': 'fixed', 'top': '0', 'left': '990px', 'width': '100%', 'height': '220px', 'overflowY': 'scroll', 'font-size': '40px', "font-weight": "bold",},
                children=[
                    dcc.Interval(
                    id='interval-component-hercules2',
                    interval=60*1000,  # Actualizar cada minuto
                    n_intervals=0
                ),
                dcc.Graph(id='colaboradores-data-hercules2', 
        ...
        html.Div(style={'position': 'fixed', 'top': '0', 'left': '800px', 'width': '100%', 'height': '220px', 'overflowY': 'scroll', 'font-size': '40px', "font-weight": "bold",},
                children=[
                dcc.Graph(id='clientes-data-hercules2', 
                          config={'displayModeBar': False},
                          style={'width': '10%', "font-weight": "bold", 'font-size': '40px'} ),
            ]),
        html.Div(
                    ...
                    html.Div(
                            ...
                            children=[
                                ...
                                html.H1(id='countdown-hercules-(2)', style={'font-size': '48px', 'margin-left': '30px', 'display': 'inlne-block',}),
                                html.P(id='message-hercules-(2)', style={'font-size': '20px', 'margin-left': '30px'}),
                            ]
                        ),
        html.Div(id="clientes_inaccesibles_hercules2"),
        html.H2(""),
...
@app.callback(
    Output('total_clientes-hercules2', 'children'),
    Input('interval-component-hercules2', 'n_intervals')
)
def update_colaboradores_data(n):
    df = cargar_clientes()
    df = pd.DataFrame(df)
    df = df[df["lugar"] == "HERCULES2"]
    
    return html.Div(f"Total de clientes: {len(df)} \n")

@app.callback(
    Output('clientes_inaccesibles_hercules2', 'children'),
    Input('interval-component-hercules2', 'n_intervals')
)
def clientes(n):    
...
@app.callback(
    Output('colaboradores-data-hercules2', 'figure'),
    Input('interval-component-hercules2', 'n_intervals')
)
def update_colaboradores_data(n):
...
@app.callback(
    Output('clientes-data-hercules2', 'figure'),
    Input('interval-component-hercules2', 'n_intervals')
)
def update_colaboradores_data(n):
    df = cargar_clientes()
...
@app.callback(
    Output('contador-hercules-(2)', 'children'),
...
@app.callback(
    Output('button-agregar-hercules-(2)', 'disabled'),  # Habilitar/deshabilitar el botón de agregar
    [Input('input-cliente-hercules-(2)', 'value'),
     Input('input-likes-hercules-(2)', 'value'),
     Input('input-comentarios-hercules-(2)', 'value'),
     Input('input-compartidos-hercules-(2)', 'value'),
     Input('input-ubicacion-hercules-(2)','value')]
)
...
@app.callback(
    Output('clear-button-hercules-(2)', 'disabled'),  # Disable the delete button if no row is selected
    [Input('data-table', 'selected_rows')]
)
def update_button_disabled2(selected_rows):
    return len(selected_rows) == 0

@app.callback(Output('data-table2-hercules-(2)', 'data'),
              Input('interval-component', 'n_intervals'))
...
@app.callback(
    Output('input-cliente-hercules-(2)', 'value'),  # Limpiar el campo de entrada
    Output('input-likes-hercules-(2)', 'value'),  # Limpiar la selección de likes
    Output('input-comentarios-hercules-(2)', 'value'),  # Limpiar la selección de comentarios
    Output('input-compartidos-hercules-(2)', 'value'),
    Output('input-ubicacion-hercules-(2)', 'value'),  # Limpiar la selección de compartidos
    [Input('button-agregar-hercules-(2)', 'n_clicks'),
     Input('clear-button-hercules-(2)', 'n_clicks')],  # Include clear-button-hercules-(2) as input trigger
...
@app.callback(
    Output('countdown-hercules-(2)', 'children'),
    Output('message-hercules-(2)', 'children'),
    Input('interval', 'n_intervals'),
    State('countdown-hercules-(2)', 'children')
)
