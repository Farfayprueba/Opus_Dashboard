...
def get_colaboradores_data():
    ...
    SELECT usuario
    FROM colaboradores
    WHERE statusData = 1 AND lugar = 'chetumal2'
    """
...
def load_data():
    ...
    try:
        ...
        df = df[df['Pagina'].apply(lambda x: x.lower()).isin([cliente['cliente'].lower() for cliente in clientes if cliente['lugar'] == "chetumal2-vespertino"])]
...
layout = html.Div(
    children=[
        html.H1("Proyecto Opus Team chetumal (2) vespertino"),
        ...
        html.Div(style={'position': 'fixed', 'top': '0', 'left': '990px', 'width': '100%', 'height': '220px', 'overflowY': 'scroll', 'font-size': '40px', "font-weight": "bold",},
                children=[
                    dcc.Interval(
                    id='interval-component-chetumal2',
                    interval=60*1000,  # Actualizar cada minuto
                    n_intervals=0
                ),
                dcc.Graph(id='colaboradores-data-chetumal2', 
        ...
        html.Div(
                    ...
                    children=[
                        html.Div(
                            id='contador-chetumal2-vespertino',
                            style={
                                "text-align": "left",
                                "margin-left": "25px",
                                "font-size": "28px",
                    ...
                    html.Div(
                            ...
                            children=[
                                ...
                                html.H1(id='countdown-chetumal2-vespertino', style={'font-size': '48px', 'margin-left': '30px', 'display': 'inlne-block',}),
                                html.P(id='message-chetumal2-vespertino', style={'font-size': '20px', 'margin-left': '30px'}),
                            ]
                        ),
        html.H2(""),
            ...
            children=[
                ...
                html.Div(
                    dash_table.DataTable(
                        id="data-table2-chetumal2-vespertino",
                        columns=columns,
...
@app.callback(
    Output('colaboradores-data-chetumal2', 'figure'),
    Input('interval-component-chetumal2', 'n_intervals')
)
...
@app.callback(
    Output('contador-chetumal2-vespertino', 'children'),
...
@app.callback(
    Output('button-agregar-chetumal2-vespertino', 'disabled'),  # Habilitar/deshabilitar el botón de agregar
    [Input('input-cliente-chetumal2-vespertino', 'value'),
     Input('input-likes-chetumal2-vespertino', 'value'),
     Input('input-comentarios-chetumal2-vespertino', 'value'),
     Input('input-compartidos-chetumal2-vespertino', 'value'),
     Input('input-ubicacion-chetumal2-vespertino','value')]
)
...
@app.callback(
    Output('clear-button-chetumal2-vespertino', 'disabled'),  # Disable the delete button if no row is selected
    [Input('data-table', 'selected_rows')]
)
def update_button_disabled2(selected_rows):
    return len(selected_rows) == 0

@app.callback(Output('data-table2-chetumal2-vespertino', 'data'),
              Input('interval-component', 'n_intervals'))
def update_data_table2(n_intervals):
    ...
    return updated_data
@app.callback(
    Output('input-cliente-chetumal2-vespertino', 'value'),  # Limpiar el campo de entrada
    Output('input-likes-chetumal2-vespertino', 'value'),  # Limpiar la selección de likes
    Output('input-comentarios-chetumal2-vespertino', 'value'),  # Limpiar la selección de comentarios
    Output('input-compartidos-chetumal2-vespertino', 'value'),
    Output('input-ubicacion-chetumal2-vespertino', 'value'),  # Limpiar la selección de compartidos
    [Input('button-agregar-chetumal2-vespertino', 'n_clicks'),
     Input('clear-button-chetumal2-vespertino', 'n_clicks')],  # Include clear-button-chetumal2-vespertino as input trigger
...
@app.callback(
    Output('countdown-chetumal2-vespertino', 'children'),
    Output('message-chetumal2-vespertino', 'children'),
    Input('interval', 'n_intervals'),
    State('countdown-chetumal2-vespertino', 'children')
)
