...
def get_colaboradores_data():
    ...
    SELECT usuario
    FROM colaboradores
    WHERE statusData = 1 AND lugar = 'c'
    """
...
def load_data():
    ...
    try:
        ...
        # Esta lista incluye los lugares que deseas filtrar (C1, C2 y C3)
        lugares_filtrar = ["C1", "C2", "C3"]
        ...
        html.Div(
                    ...
                    html.Div(
                            ...
                            children=[
                                ...
                                html.H1(id='countdown-c', style={'font-size': '48px', 'margin-left': '30px', 'display': 'inlne-block',}),
                                html.P(id='message-c', style={'font-size': '20px', 'margin-left': '30px'}),
                            ]
                        ),
        html.H2(""),
...
@app.callback(
    Output('colaboradores-data-c', 'figure'),
    Input('interval-component-c', 'n_intervals')
)
...
@app.callback(
    Output('button-agregar-c', 'disabled'),  # Habilitar/deshabilitar el botón de agregar
    [Input('input-cliente-c', 'value'),
     Input('input-likes-c', 'value'),
     Input('input-comentarios-c', 'value'),
     Input('input-compartidos-c', 'value'),
     Input('iinput-ubicacion-c','value')]
)
...
@app.callback(
    Output('clear-button-c', 'disabled'),  # Disable the delete button if no row is selected
    [Input('data-table', 'selected_rows')]
)
def update_button_disabled2(selected_rows):
    return len(selected_rows) == 0

@app.callback(Output('data-table2-c', 'data'),
              Input('interval-component', 'n_intervals'))
def update_data_table2(n_intervals):
    ...
    return updated_data
@app.callback(
    Output('input-cliente-c', 'value'),  # Limpiar el campo de entrada
    Output('input-likes-c', 'value'),  # Limpiar la selección de likes
    Output('input-comentarios-c', 'value'),  # Limpiar la selección de comentarios
    Output('input-compartidos-c', 'value'),
    Output('iinput-ubicacion-c', 'value'),  # Limpiar la selección de compartidos
    [Input('button-agregar-c', 'n_clicks'),
     Input('clear-button-c', 'n_clicks')],  # Include clear-button-c as input trigger
...
@app.callback(
    Output('countdown-c', 'children'),
    Output('message-c', 'children'),
    Input('interval', 'n_intervals'),
    State('countdown-c', 'children')
)
