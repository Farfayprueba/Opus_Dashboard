...
def clientes_inaccesibles():
    ...
    SELECT nombre
    FROM clientes
    WHERE Opus = 3 AND lugar = 'HERCULES4' AND estatus = 'Activo'
    """
...
def get_colaboradores_data():
    ...
    SELECT usuario
    FROM colaboradores
    WHERE statusData = 1 AND lugar = 'hercules4'
    """
    mydbs = mysql.connector.connect(
        host='rimgsa.com',
        port=3306,
...
def delete_rows(df):
    ...
    for cliente in clientes_guardados:
        ...
        rows_to_delete = df[df.apply(lambda row: (
            row["Pagina"].lower() == cliente["nombre"].lower() and
            (
                (
                    row["Reshare"] == "1" and
                    (
                        (int(compartidos_goal) < 20 and int(row["Compartidos"]) >= int(compartidos_goal)) or
                        (int(compartidos_goal) > 20 and int(row["Comentarios"]) >= 10 and int(row["Compartidos"]) >= 20)
                    )
                ) or
                (
                    row["Reshare"] == "0" and
                    row["Compartidos"] >= compartidos_goal and
                    row["Comentarios"] >= comentarios_goal
                )
            )
        ), axis=1)]
...
def load_data():
    ...
    try:
        ...
        df = df[df['Pagina'].apply(lambda x: x.lower()).isin([cliente['cliente'].lower() for cliente in clientes if cliente['lugar'] == "HERCULES4"])]
...
layout = html.Div(
    children=[
        html.H1("Proyecto Opus Team Hercules (4)"),
        html.H1(id="total_clientes-hercules4", #cambiar id
                style={'float': 'right',
                    "margin-right":"50px",
                    "font-size": "28px",
                    "font-weight": "bold",
        ...
        html.Div(style={'position': 'fixed', 'top': '0', 'left': '990px', 'width': '100%', 'height': '220px', 'overflowY': 'scroll', 'font-size': '40px', "font-weight": "bold",},
                children=[
                    dcc.Interval(
                    id='interval-component-hercules4',
                    interval=60*1000,  # Actualizar cada minuto
                    n_intervals=0
                ),
                dcc.Graph(id='colaboradores-data-hercules4', 
        ...
        html.Div(style={'position': 'fixed', 'top': '0', 'left': '800px', 'width': '100%', 'height': '220px', 'overflowY': 'scroll', 'font-size': '40px', "font-weight": "bold",},
                children=[
                dcc.Graph(id='clientes-data-hercules4', 
                          config={'displayModeBar': False},
                          style={'width': '10%', "font-weight": "bold", 'font-size': '40px'} ),
            ]),
        html.Div(
                    ...
                    html.Div(
                            ...
                            children=[
                                ...
                                html.H1(id='countdown-hercules-(4)', style={'font-size': '48px', 'margin-left': '30px', 'display': 'inlne-block',}),
                                html.P(id='message-hercules-(4)', style={'font-size': '20px', 'margin-left': '30px'}),
                            ]
                        ),
        html.Div(id="clientes_inaccesibles_hercules4"),
        html.H2(""),
...
@app.callback(
    Output('total_clientes-hercules4', 'children'),
    Input('interval-component-hercules4', 'n_intervals')
)
def update_colaboradores_data(n):
    df = cargar_clientes()
    df = pd.DataFrame(df)
    df = df[df["lugar"] == "HERCULES4"]
    
    return html.Div(f"Total de clientes: {len(df)} \n")

@app.callback(
    Output('clientes_inaccesibles_hercules4', 'children'),
    Input('interval-component-hercules4', 'n_intervals')
)
def clientes(n):    
    ...
    return table
@app.callback(
    Output('colaboradores-data-hercules4', 'figure'),
    Input('interval-component-hercules4', 'n_intervals')
)
def update_colaboradores_data(n):
...
@app.callback(
    Output('clientes-data-hercules4', 'figure'),
    Input('interval-component-hercules4', 'n_intervals')
)
def update_colaboradores_data(n):
    df = cargar_clientes()
...
@app.callback(
    Output('contador-hercules-(4)', 'children'),
...
@app.callback(
    Output('button-agregar-hercules-(4)', 'disabled'),  # Habilitar/deshabilitar el botón de agregar
    [Input('input-cliente-hercules-(4)', 'value'),
     Input('input-likes-hercules-(4)', 'value'),
     Input('input-comentarios-hercules-(4)', 'value'),
     Input('input-compartidos-hercules-(4)', 'value'),
     Input('input-ubicacion-hercules-(4)','value')]
)
...
@app.callback(
    Output('clear-button-hercules-(4)', 'disabled'),  # Disable the delete button if no row is selected
    [Input('data-table', 'selected_rows')]
)
def update_button_disabled2(selected_rows):
    return len(selected_rows) == 0

@app.callback(Output('data-table2-hercules-(4)', 'data'),
              Input('interval-component', 'n_intervals'))
...
@app.callback(
    Output('input-cliente-hercules-(4)', 'value'),  # Limpiar el campo de entrada
    Output('input-likes-hercules-(4)', 'value'),  # Limpiar la selección de likes
    Output('input-comentarios-hercules-(4)', 'value'),  # Limpiar la selección de comentarios
    Output('input-compartidos-hercules-(4)', 'value'),
    Output('input-ubicacion-hercules-(4)', 'value'),  # Limpiar la selección de compartidos
    [Input('button-agregar-hercules-(4)', 'n_clicks'),
     Input('clear-button-hercules-(4)', 'n_clicks')],  # Include clear-button-hercules-(4) as input trigger
...
@app.callback(
    Output('countdown-hercules-(4)', 'children'),
    Output('message-hercules-(4)', 'children'),
    Input('interval', 'n_intervals'),
    State('countdown-hercules-(4)', 'children')
)
